...
class AdvanceCombatAuraModule : Module("AdvanceCombatAura", ModuleCategory.Combat) {
    ...
    private var strafeSpeed by floatValue("strafe_speed", 2.0f, 1f..3f)
    private var strafeRadius by floatValue("strafe_radius", 3.0f, 2f..6f)

    // These need to be mutable (var)
    private var lastAttackTime: Long = 0L
    private var tpCooldown: Long = 0L

    // APS and APS Boost
    private var apsBoostEnabled by boolValue("APS Booster", false)
    private var baseAPS by floatValue("Base APS", 12f, 5f..20f)
    private var boostAPS by floatValue("Boost Value", 18f, 12f..25f)
    private var boostChance by intValue("Boost Chance", 20, 0..100) // % chance for boost hit

    // APS value needs to be mutable (var)
    private var apsValue by floatValue("Value", 12f, 5f..20f)
    ...
    override fun beforePacketBound(interceptablePacket: InterceptablePacket) {
        ...
        if (packet is PlayerAuthInputPacket) {
            ...
            val aps = if (apsBoostEnabled && (0..100).random() < boostChance) boostAPS else baseAPS
            val minAttackDelay = (1000L / aps).toLong()
            ...
            if (packet.tick % attackInterval == 0L && (currentTime - lastAttackTime) >= minAttackDelay) {
                ...
                closestEntities.forEach { entity ->
                    smoothRotateTo(entity) // Smoothly rotate to entity like Horion
    ...
    private fun smoothRotateTo(target: Entity, smoothing: Float = 0.3f) {
        val player = session.localPlayer
        val diffX = target.vec3Position.x - player.vec3Position.x
        val diffZ = target.vec3Position.z - player.vec3Position.z
        val diffY = (target.vec3Position.y + 1.62f) - (player.vec3Position.y + 1.62f)

        val distanceXZ = kotlin.math.sqrt(diffX * diffX + diffZ * diffZ)

        val targetYaw = Math.toDegrees(kotlin.math.atan2(-diffX.toDouble(), diffZ.toDouble())).toFloat()
        val targetPitch = Math.toDegrees(-kotlin.math.atan2(diffY.toDouble(), distanceXZ.toDouble())).toFloat()

        // Smooth rotation (lerp)
        val yaw = lerpAngle(player.vec3Rotation.y, targetYaw, smoothing)
        val pitch = lerpAngle(player.vec3Rotation.x, targetPitch, smoothing)

        // Correct the rotation assignment here
