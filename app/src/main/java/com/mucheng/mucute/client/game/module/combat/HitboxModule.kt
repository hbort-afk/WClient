...
class HitboxModule : Module("Hitbox", ModuleCategory.Combat) {
    ...
    private val hitboxWidth by floatValue("Width", 5.0f, 0.5f..20f) // Increased width range
    private val hitboxHeight by floatValue("Height", 5.0f, 0.5f..20f) // Increased height range
    ...
    override fun beforePacketBound(interceptablePacket: InterceptablePacket) {
        ...
        if (packet is PlayerAuthInputPacket) {
            ...
            if (session.localPlayer.tickExists % 20 == 0L) { // Increased frequency for quicker updates
    ...
    override fun onDisabled() {
        ...
        if (isSessionCreated) {
            session.level.entityMap.values.forEach { entity ->
                if (entity.isTarget()) {
                    val metadata = EntityDataMap()
                    metadata.put(EntityDataTypes.WIDTH, 0.6f) // Reset to default size
    ...
    private fun visualizeHitboxWithParticles(entity: Entity) {
        ...
        repeat(particleCount) { i ->
            ...
            session.clientBound(EntityEventPacket().apply {
                runtimeEntityId = entity.runtimeEntityId
                type = EntityEventType.LOVE_PARTICLES // You could also use other types like 'BLOCK_CRACK'
                data = 0
            })
        }
    }
